  ...
  return (
    <form onSubmit={handleSubmit(submit)} className="space-y-4">
      ...
      <div>
        ...
        <input {...register('name', { required: 'Required' })} className={errors.name ? 'input-error' : 'input'} />
        {errors.name && <p className="mt-1 text-xs text-red-500">{errors.name.message}</p>}
      </div>
      <div>
        <label className="label">Duration (weeks)</label>
        <input type="number" min={1} {...register('duration_weeks', { required: 'Required', min: { value: 1, message: 'Min 1 week' } })} className={errors.duration_weeks ? 'input-error' : 'input'} />
      ...
      <div className="flex gap-3 pt-2">
        ...
        <button type="submit" disabled={isSubmitting} className="btn-primary flex-1">{isSubmitting ? 'Saving…' : 'Save course'}</button>
      </div>
    </form>
  )
}
