...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    async function load() {
      const [summaryRes, svRes] = await Promise.all([
        supabase.from('intern_payment_summary').select('*'),
        supabase.from('supervisors').select('id, full_name'),
      ])
  ...
  const totalInterns = summary.length
  ...
  // Group by supervisor for progress bars
  const bySupervisor = supervisors
    .map((sv) => {
      ...
      const pct = regs.length ? Math.round((paid / regs.length) * 100) : 0
      return { ...sv, total: regs.length, paid, pct }
    })
    .filter((s) => s.total > 0)
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-4 gap-4">
        <MetricCard label="Total interns" value={totalInterns} icon={IconUserCheck} />
        <MetricCard label="Supervisors" value={totalSupervisors} icon={IconUsers} />
