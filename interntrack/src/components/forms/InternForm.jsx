...
export default function InternForm({ onSubmit, onClose, supervisors, courses }) {
  const { user } = useAuth()
  const {
    register,
    handleSubmit,
  ...
  async function submit(data) {
    ...
    try {
      await onSubmit(
        {
          ...
          school_of_origin: data.school_of_origin,
        ...
        },
        {
          ...
          amount_due: activeRate.agreed_amount,
          currency: activeRate.currency || 'RWF',
      ...
      toast.error(err.message || 'Failed to register intern')
    }
  }
  ...
  return (
    <form onSubmit={handleSubmit(submit)} className="space-y-4">
      ...
      <div>
        ...
        <select
          {...register('supervisor_id', { required: 'Required' })}
          className={errors.supervisor_id ? 'input-error' : 'input'}
        >
          <option value="">Select supervisor…</option>
          {supervisors.map((s) => (
            <option key={s.id} value={s.id}>
              {s.full_name} — {s.school_facility}
            </option>
          ))}
        </select>
      ...
      <div>
        ...
        <select
          ...
          className={errors.course_id ? 'input-error' : 'input'}
        >
          <option value="">Select course…</option>
          {courses.map((c) => (
            <option key={c.id} value={c.id}>
              {c.name} ({c.duration_weeks}w)
            </option>
          ))}
        </select>
      ...
      <div>
        ...
        <div
          className={`input bg-gray-50 font-medium ${
            activeRate ? 'text-gray-800' : 'text-gray-400 italic'
          }`}
        >
          {rateLabel}
        </div>
